...
<body>
	<form name="client_form" action="clients.php" method="post">
		...
		<p>VAT client: <input type="text" name="VAT_client"/></p>
	...
	function validateAndSend() {
		if (client_form.VAT_client.value != '') {
			client_form.submit();
		}
		else if (client_form.client_name.value != '') {
			client_form.submit();
		}
		else if (client_form.client_address_street.value != '') {
			client_form.submit();
		}
		else if (client_form.client_address_city.value != '') {
			client_form.submit();
		}
		else if (client_form.client_address_zip.value != '') {
			client_form.submit();
			}
		else {
			alert('You have to fill at least one of the entries, complete VAT, partial Client Name and/or the address.');
			return false;
		}
	}
  </script>
